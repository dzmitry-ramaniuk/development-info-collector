...
3. [Как главы связаны между собой](#как-главы-связаны-между-собой)
4. [Рекомендации по изучению](#рекомендации-по-изучению)
...
Этот файл — обзорная страница и дорожная карта раздела. Подробные учебные главы находятся в каталоге [`multithreading/`](multithreading/README.md); это продолжение текущего материала, а не отдельный параллельный раздел. Главы можно изучать последовательно или открывать нужную тему напрямую. Каждая глава содержит практические примеры кода, объяснения концепций и типичные паттерны использования.

Подробное оглавление раздела также доступно в [multithreading/README.md](multithreading/README.md).
