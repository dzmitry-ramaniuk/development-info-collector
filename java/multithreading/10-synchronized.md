...
3. [Синтаксис и формы использования](#синтаксис-и-формы-использования)
...
4. [Семантика happens-before](#семантика-happens-before)
5. [Реентерабельность synchronized](#реентерабельность-synchronized)
6. [Взаимодействие с wait, notify и notifyAll](#взаимодействие-с-wait-notify-и-notifyall)
...
8. [Производительность и overhead](#производительность-и-overhead)
   - [Стоимость synchronized](#стоимость-synchronized)
   - [Рекомендации по производительности](#рекомендации-по-производительности)
9. [Сравнение с другими механизмами](#сравнение-с-другими-механизмами)
   - [synchronized vs ReentrantLock](#synchronized-vs-reentrantlock)
   - [synchronized vs volatile](#synchronized-vs-volatile)
   - [synchronized vs Atomic классы](#synchronized-vs-atomic-классы)
10. [Best Practices](#best-practices)
   - [1. Используйте приватные final объекты-мониторы](#1-используйте-приватные-final-объекты-мониторы)
   - [2. Минимизируйте область синхронизации](#2-минимизируйте-область-синхронизации)
   - [3. Документируйте стратегию блокировки](#3-документируйте-стратегию-блокировки)
   - [4. Избегайте блокировки на публичных объектах](#4-избегайте-блокировки-на-публичных-объектах)
   - [5. Соблюдайте единый порядок блокировок](#5-соблюдайте-единый-порядок-блокировок)
   - [6. Не вызывайте чужие методы под блокировкой](#6-не-вызывайте-чужие-методы-под-блокировкой)
   - [7. Используйте synchronized для простых случаев](#7-используйте-synchronized-для-простых-случаев)
11. [Типичные ошибки и антипаттерны](#типичные-ошибки-и-антипаттерны)
   - [1. Блокировка на изменяемых объектах](#1-блокировка-на-изменяемых-объектах)
   - [2. Блокировка на примитивных обёртках](#2-блокировка-на-примитивных-обёртках)
   - [3. Забытая синхронизация в геттерах](#3-забытая-синхронизация-в-геттерах)
   - [4. Использование wait() без цикла](#4-использование-wait-без-цикла)
   - [5. Deadlock через неупорядоченные блокировки](#5-deadlock-через-неупорядоченные-блокировки)
   - [6. Синхронизация на null или на разных объектах](#6-синхронизация-на-null-или-на-разных-объектах)
12. [Практические примеры](#практические-примеры)
   - [Пример 1: Thread-safe Cache](#пример-1-thread-safe-cache)
   - [Пример 2: Producer-Consumer с wait/notify](#пример-2-producer-consumer-с-waitnotify)
   - [Пример 3: Ленивая инициализация (Double-Checked Locking)](#пример-3-ленивая-инициализация-double-checked-locking)
   - [Пример 4: Счётчик с отдельными блокировками для чтения и записи](#пример-4-счётчик-с-отдельными-блокировками-для-чтения-и-записи)
   - [Пример 5: Thread-safe Singleton](#пример-5-thread-safe-singleton)
