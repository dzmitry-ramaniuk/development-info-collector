...
3. [Проблема, которую решает](#проблема-которую-решает)
   - [Проблема: Несовместимые интерфейсы](#проблема-несовместимые-интерфейсы)
   - [Решение: Adapter](#решение-adapter)
4. [Структура паттерна](#структура-паттерна)
5. [Типы адаптеров](#типы-адаптеров)
   - [1. Адаптер объектов (Object Adapter) - использует композицию](#1-адаптер-объектов-object-adapter---использует-композицию)
   - [2. Адаптер классов (Class Adapter) - использует множественное наследование](#2-адаптер-классов-class-adapter---использует-множественное-наследование)
6. [Реализация](#реализация)
   - [Пример 1: Адаптер для логирования](#пример-1-адаптер-для-логирования)
   - [Пример 2: Адаптер для различных форматов данных](#пример-2-адаптер-для-различных-форматов-данных)
   - [Пример 3: Адаптер для различных платежных систем](#пример-3-адаптер-для-различных-платежных-систем)
   - [Пример 4: Двунаправленный адаптер](#пример-4-двунаправленный-адаптер)
