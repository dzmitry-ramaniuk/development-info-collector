...
3. [Проблема, которую решает](#проблема-которую-решает)
   - [Проблема: Прямые вызовы методов](#проблема-прямые-вызовы-методов)
   - [Решение: Command](#решение-command)
...
6. [Macro Commands](#macro-commands)
7. [Примеры из JDK](#примеры-из-jdk)
   - [1. java.lang.Runnable](#1-javalangrunnable)
   - [2. javax.swing.Action](#2-javaxswingaction)
   - [3. Thread и Thread Pool](#3-thread-и-thread-pool)
...
9. [Вопросы на собеседовании](#вопросы-на-собеседовании)
   - [1. Что такое паттерн Command?](#1-что-такое-паттерн-command)
   - [2. В чем отличие Command от Strategy?](#2-в-чем-отличие-command-от-strategy)
   - [3. Как реализовать undo/redo функциональность?](#3-как-реализовать-undoredo-функциональность)
   - [4. Когда следует использовать паттерн Command?](#4-когда-следует-использовать-паттерн-command)
   - [5. Что такое макрокоманды?](#5-что-такое-макрокоманды)
   - [6. Как Command используется в многопоточном окружении?](#6-как-command-используется-в-многопоточном-окружении)
   - [7. В чем разница между Command и Callback?](#7-в-чем-разница-между-command-и-callback)
   - [8. Как хранить историю команд с минимальным использованием памяти?](#8-как-хранить-историю-команд-с-минимальным-использованием-памяти)
   - [9. Можно ли комбинировать Command с другими паттернами?](#9-можно-ли-комбинировать-command-с-другими-паттернами)
   - [10. Как тестировать команды?](#10-как-тестировать-команды)
