...
2. [Общие принципы тестирования](#общие-принципы-тестирования)
   - [Пирамида тестирования](#пирамида-тестирования)
   - [F.I.R.S.T. принципы](#first-принципы)
   - [Arrange-Act-Assert (AAA)](#arrange-act-assert-aaa)
3. [Типичные ошибки](#типичные-ошибки)
   - [1. Избыточные моки](#1-избыточные-моки)
   - [2. Использование H2 вместо реальной БД](#2-использование-h2-вместо-реальной-бд)
   - [3. Thread.sleep() в тестах](#3-threadsleep-в-тестах)
   - [4. Непредсказуемость времени и UUID](#4-непредсказуемость-времени-и-uuid)
   - [5. Отсутствие очистки данных между тестами](#5-отсутствие-очистки-данных-между-тестами)
   - [6. @DirtiesContext без необходимости](#6-dirtiescontext-без-необходимости)
   - [7. Верификация вызовов без нужды](#7-верификация-вызовов-без-нужды)
   - [8. Сложные фикстуры](#8-сложные-фикстуры)
   - [9. Зависимость от порядка выполнения тестов](#9-зависимость-от-порядка-выполнения-тестов)
   - [10. Плохие названия тестов](#10-плохие-названия-тестов)
   - [11. Недостаточное покрытие ошибок](#11-недостаточное-покрытие-ошибок)
   - [12. Отсутствие проверки сериализации JSON](#12-отсутствие-проверки-сериализации-json)
   - [13. Несогласованность данных при параллельном запуске](#13-несогласованность-данных-при-параллельном-запуске)
   - [14. Отсутствие документации тестов](#14-отсутствие-документации-тестов)
4. [Антипаттерны в тестировании](#антипаттерны-в-тестировании)
   - [The Liar](#the-liar)
   - [The Giant](#the-giant)
   - [The Slow Poke](#the-slow-poke)
   - [The Secret Catcher](#the-secret-catcher)
5. [Best practices](#best-practices)
   - [1. Тестируй поведение, а не реализацию](#1-тестируй-поведение-а-не-реализацию)
   - [2. Один тест — одна концепция](#2-один-тест--одна-концепция)
   - [3. Используй явные assertions](#3-используй-явные-assertions)
   - [4. Тестируй граничные случаи](#4-тестируй-граничные-случаи)
   - [5. Избегай логики в тестах](#5-избегай-логики-в-тестах)
   - [6. Не тестируй сторонние библиотеки](#6-не-тестируй-сторонние-библиотеки)
6. [Организация тестового кода](#организация-тестового-кода)
   - [Структура пакетов](#структура-пакетов)
   - [Именование тестовых классов](#именование-тестовых-классов)
   - [Группировка тестов](#группировка-тестов)
7. [Производительность тестов](#производительность-тестов)
   - [Измерение времени](#измерение-времени)
   - [Параллельное выполнение (JUnit 5)](#параллельное-выполнение-junit-5)
   - [Оптимизация Spring контекста](#оптимизация-spring-контекста)
