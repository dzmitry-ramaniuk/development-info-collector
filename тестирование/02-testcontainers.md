...
2. [Введение и основы Testcontainers](#введение-и-основы-testcontainers)
   - [Проблема классического подхода](#проблема-классического-подхода)
   - [Решение с Testcontainers](#решение-с-testcontainers)
   - [Архитектура Testcontainers](#архитектура-testcontainers)
3. [Установка и зависимости](#установка-и-зависимости)
   - [Требования](#требования)
   - [Актуальность версий](#актуальность-версий)
   - [Базовая зависимость (Maven)](#базовая-зависимость-maven)
   - [Базовая зависимость (Gradle)](#базовая-зависимость-gradle)
   - [Специализированные модули](#специализированные-модули)
4. [Базовые примеры использования](#базовые-примеры-использования)
   - [Пример 1: Универсальный контейнер (GenericContainer)](#пример-1-универсальный-контейнер-genericcontainer)
   - [Пример 2: PostgreSQL контейнер](#пример-2-postgresql-контейнер)
   - [Пример 3: Несколько контейнеров](#пример-3-несколько-контейнеров)
5. [Специализированные модули](#специализированные-модули-1)
...
6. [Интеграция с JUnit 5](#интеграция-с-junit-5)
   - [Аннотация @Testcontainers](#аннотация-testcontainers)
   - [Контейнер на метод vs на класс](#контейнер-на-метод-vs-на-класс)
   - [Singleton контейнер для нескольких тестовых классов](#singleton-контейнер-для-нескольких-тестовых-классов)
7. [Продвинутые возможности](#продвинутые-возможности)
   - [Ожидание готовности контейнера (Wait Strategies)](#ожидание-готовности-контейнера-wait-strategies)
   ...
   - [Логирование контейнера](#логирование-контейнера)
   - [Сеть между контейнерами](#сеть-между-контейнерами)
   - [Переиспользование контейнеров (Reusable Containers)](#переиспользование-контейнеров-reusable-containers)
8. [Конфигурация и настройка](#конфигурация-и-настройка)
   - [Конфигурационный файл](#конфигурационный-файл)
   - [Переменные окружения](#переменные-окружения)
   - [Использование локального Docker Registry](#использование-локального-docker-registry)
   - [Кастомизация Docker-образа](#кастомизация-docker-образа)
9. [Лучшие практики](#лучшие-практики)
   - [1. Используйте специализированные модули](#1-используйте-специализированные-модули)
   - [2. Закрепляйте версии образов](#2-закрепляйте-версии-образов)
   - [3. Переиспользуйте контейнеры для класса](#3-переиспользуйте-контейнеры-для-класса)
   - [4. Используйте @DynamicPropertySource для Spring](#4-используйте-dynamicpropertysource-для-spring)
   - [5. Используйте init scripts для начальных данных](#5-используйте-init-scripts-для-начальных-данных)
   - [6. Настройте логирование](#6-настройте-логирование)
   - [7. Используйте профили для отключения в локальной разработке](#7-используйте-профили-для-отключения-в-локальной-разработке)
   - [8. Обрабатывайте ресурсы правильно](#8-обрабатывайте-ресурсы-правильно)
   - [9. Оптимизируйте время запуска тестов](#9-оптимизируйте-время-запуска-тестов)
   - [10. Документируйте зависимости от Docker](#10-документируйте-зависимости-от-docker)
10. [Вопросы для самопроверки](#вопросы-для-самопроверки)
   ...
   - [Вопросы на отладку](#вопросы-на-отладку)
...
- Java 17+ (рекомендуется для современного стека)
- Docker Desktop (Windows/Mac) или Docker Engine (Linux)
- Maven или Gradle для управления зависимостями

### Актуальность версий

> Актуально на **март 2026**: используйте единую переменную `testcontainers.version` в сборке и регулярно сверяйте её с официальным changelog Testcontainers.

- Минимальная рекомендуемая платформа: **Java 17+**
- Версии Docker-образов в примерах закрепляйте явно (без `latest`)
- Обновляйте версии централизованно через BOM/переменные Maven или Gradle
...
void testNetworking() {
    ...
    try (GenericContainer<?> postgres = new GenericContainer<>("postgres:16-alpine")
            .withNetwork(network)
...
# Для legacy-настроек Docker Desktop (не рекомендуется для новых окружений)
# Включайте только в контролируемой среде, учитывая риски безопасности
...
```java
// Хорошо — явная версия
PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:16.2-alpine");
...
class BestTest {
    @Container
    private PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:16-alpine");
