...
2. [Концепция тестовых двойников](#концепция-тестовых-двойников)
   - [Типы тестовых двойников](#типы-тестовых-двойников)
3. [Основные аннотации Mockito](#основные-аннотации-mockito)
   - [Зависимость (Maven)](#зависимость-maven)
   - [Таблица аннотаций](#таблица-аннотаций)
   - [Пример использования аннотаций](#пример-использования-аннотаций)
4. [Основное API Mockito](#основное-api-mockito)
   - [Создание моков](#создание-моков)
5. [Стаббинг поведения](#стаббинг-поведения)
   - [Базовый стаббинг с `when().thenReturn()`](#базовый-стаббинг-с-whenthenreturn)
   - [Множественные вызовы](#множественные-вызовы)
   - [Стаббинг с любыми аргументами (Matchers)](#стаббинг-с-любыми-аргументами-matchers)
   - [Стаббинг с вычислением на основе аргументов](#стаббинг-с-вычислением-на-основе-аргументов)
   - [Стаббинг void методов](#стаббинг-void-методов)
6. [Проверка вызовов (Verification)](#проверка-вызовов-verification)
   - [Базовая проверка](#базовая-проверка)
   - [Различные режимы проверки](#различные-режимы-проверки)
   - [Проверка порядка вызовов](#проверка-порядка-вызовов)
   - [Проверка отсутствия взаимодействий](#проверка-отсутствия-взаимодействий)
7. [ArgumentCaptor — захват аргументов](#argumentcaptor--захват-аргументов)
   - [Базовое использование](#базовое-использование)
   - [Захват множественных вызовов](#захват-множественных-вызовов)
   - [С аннотацией @Captor](#с-аннотацией-captor)
8. [Исключения и альтернативные ответы](#исключения-и-альтернативные-ответы)
   - [Выбрасывание исключений](#выбрасывание-исключений)
   - [Исключения для void методов](#исключения-для-void-методов)
   - [Последовательные ответы](#последовательные-ответы)
9. [Spy — частичные моки](#spy--частичные-моки)
   - [Создание Spy](#создание-spy)
   - [Стаббинг методов Spy](#стаббинг-методов-spy)
   - [Разница между when() и doReturn() для Spy](#разница-между-when-и-doreturn-для-spy)
   - [Spy с аннотацией @Spy](#spy-с-аннотацией-spy)
10. [Пример реального теста](#пример-реального-теста)
11. [Распространённые ошибки](#распространённые-ошибки)
   - [1. Отсутствие аннотации @ExtendWith(MockitoExtension.class)](#1-отсутствие-аннотации-extendwithmockitoextensionclass)
   - [2. Неверное использование when() с spy](#2-неверное-использование-when-с-spy)
   - [3. Избыточные моки](#3-избыточные-моки)
   - [4. Смешивание матчеров и конкретных значений](#4-смешивание-матчеров-и-конкретных-значений)
   - [5. Несогласованные equals() в проверках](#5-несогласованные-equals-в-проверках)
   - [6. Проверка реализации вместо поведения](#6-проверка-реализации-вместо-поведения)
12. [Best practices](#best-practices)
   - [1. Не мокай то, что не нужно](#1-не-мокай-то-что-не-нужно)
   - [2. Проверяй результат, а не внутреннюю реализацию](#2-проверяй-результат-а-не-внутреннюю-реализацию)
   - [3. Изолируй внешний мир](#3-изолируй-внешний-мир)
   - [4. Для сложных зависимостей используй @Spy или Fake](#4-для-сложных-зависимостей-используй-spy-или-fake)
   - [5. Именуй тесты по принципу](#5-именуй-тесты-по-принципу)
   - [6. Используй @DisplayName для читаемости](#6-используй-displayname-для-читаемости)
   - [7. Группируй связанные тесты с @Nested](#7-группируй-связанные-тесты-с-nested)
   - [8. Избегай статических моков без необходимости](#8-избегай-статических-моков-без-необходимости)
   - [9. Не тестируй приватные методы напрямую](#9-не-тестируй-приватные-методы-напрямую)
   - [10. Очищай ресурсы после тестов](#10-очищай-ресурсы-после-тестов)
...
[← Назад к разделу Тестирование](README.html)
