...
2. [Введение и основы JUnit](#введение-и-основы-junit)
   - [История и версии](#история-и-версии)
   - [Архитектура JUnit 5](#архитектура-junit-5)
   - [Базовая зависимость (Maven)](#базовая-зависимость-maven)
   - [Простейший тест](#простейший-тест)
3. [Сравнение JUnit 4 и JUnit 5](#сравнение-junit-4-и-junit-5)
   - [Основные отличия](#основные-отличия)
   - [Миграция аннотаций](#миграция-аннотаций)
   - [Пример миграции теста](#пример-миграции-теста)
   - [Ключевые преимущества JUnit 5](#ключевые-преимущества-junit-5)
   - [Практические советы по миграции](#практические-советы-по-миграции)
   - [Конфигурация для поддержки обеих версий](#конфигурация-для-поддержки-обеих-версий)
   - [Вывод](#вывод)
4. [Аннотации и жизненный цикл тестов](#аннотации-и-жизненный-цикл-тестов)
   - [Основные аннотации](#основные-аннотации)
   - [Дополнительные аннотации](#дополнительные-аннотации)
   - [Жизненный цикл экземпляра теста](#жизненный-цикл-экземпляра-теста)
5. [Assertions — утверждения](#assertions--утверждения)
   - [Базовые assertions](#базовые-assertions)
   - [Assertions для массивов и коллекций](#assertions-для-массивов-и-коллекций)
   - [Assertions для исключений](#assertions-для-исключений)
   - [Групповые assertions](#групповые-assertions)
   - [Assertions с сообщениями](#assertions-с-сообщениями)
6. [Параметризованные тесты](#параметризованные-тесты)
   - [Зависимость для параметризованных тестов](#зависимость-для-параметризованных-тестов)
   - [`@ValueSource`](#valuesource)
   - [`@CsvSource`](#csvsource)
   - [`@CsvFileSource`](#csvfilesource)
   - [`@MethodSource`](#methodsource)
   - [`@EnumSource`](#enumsource)
   - [Пользовательские имена для параметризованных тестов](#пользовательские-имена-для-параметризованных-тестов)
7. [Тестовые наборы и теги](#тестовые-наборы-и-теги)
   - [Теги (`@Tag`)](#теги-tag)
   - [Вложенные тесты (`@Nested`)](#вложенные-тесты-nested)
   - [Условное выполнение тестов](#условное-выполнение-тестов)
8. [Расширения (Extensions)](#расширения-extensions)
   - [Встроенные расширения](#встроенные-расширения)
   - [Пользовательские расширения](#пользовательские-расширения)
   - [Регистрация расширений](#регистрация-расширений)
9. [Интеграция с инструментами сборки](#интеграция-с-инструментами-сборки)
   - [Maven](#maven)
   - [Gradle](#gradle)
10. [Лучшие практики](#лучшие-практики)
   - [Именование тестов](#именование-тестов)
   - [Структура тестов: AAA паттерн](#структура-тестов-aaa-паттерн)
   - [Принцип единственной ответственности](#принцип-единственной-ответственности)
   - [Независимость тестов](#независимость-тестов)
   - [Тестирование исключений](#тестирование-исключений)
   - [Использование тестовых данных](#использование-тестовых-данных)
   - [Покрытие кода тестами](#покрытие-кода-тестами)
   - [Мокирование и стабы](#мокирование-и-стабы)
11. [Вопросы для самопроверки](#вопросы-для-самопроверки)
   ...
   - [Вопросы на интеграцию](#вопросы-на-интеграцию)
...
JUnit 5 не является просто новой версией JUnit 4 — это полностью переработанная архитектура. Она решает множество проблем, которые существовали в JUnit 4, и добавляет расширяемость и гибкость.

### Основные отличия

| Особенность             | JUnit 4                              | JUnit 5                                |
| ----------------------- | ------------------------------------ | -------------------------------------- |
| Базовый пакет           | `org.junit`                          | `org.junit.jupiter.api`                |
| Запуск тестов           | `@RunWith`                           | `@ExtendWith` (Extensions API)         |
| Категории               | `@Category`                          | `@Tag`                                 |
| Правила                 | `@Rule`, `@ClassRule`                | Extensions API                         |
| Ассерты                 | `Assert.*`                           | `Assertions.*` с лямбдами              |
| Параметризованные тесты | Отдельная зависимость `junit-params` | Встроены (`@ParameterizedTest`)        |
| Совместимость           | Только JUnit 4                       | Через JUnit Vintage                    |
| Параллельное выполнение | Ограничено                           | Полноценная поддержка (JUnit Platform) |
| Display name            | Отсутствует                          | `@DisplayName` для читаемых названий   |
| Assumptions             | Есть                                 | Есть (улучшено)                        |
...
| `@BeforeClass`     | `@BeforeAll`    | Выполняется один раз перед всеми       |
| `@AfterClass`      | `@AfterAll`     | Выполняется один раз после всех        |
| `@Ignore`          | `@Disabled`     | Отключает тест                         |
| `@Category`        | `@Tag`          | Категоризация тестов                   |
| `@RunWith`         | `@ExtendWith`   | Подключение расширений                 |
| `@Rule`            | `@ExtendWith`   | Заменено на Extensions API             |
...
### Ключевые преимущества JUnit 5

1. **Расширяемость через Extensions API** — можно создавать собственные расширения (логирование, ресурсы, конфигурация).
2. **Параметризованные тесты встроены** — нет необходимости подключать отдельные библиотеки.
3. **Поддержка параллельного запуска** — повышает скорость CI/CD пайплайнов.
4. **Совместимость через Vintage** — можно мигрировать постепенно.
5. **Лучшие отчёты и `@DisplayName`** — тесты читаемы и понятны.
6. **Улучшенные Assertions** — поддержка лямбда-выражений и группировка с `assertAll()`.
7. **Условное выполнение** — встроенные аннотации для запуска тестов на определённых ОС, версиях Java и т.д.

### Практические советы по миграции

1. Начинай с включения **JUnit Vintage**, чтобы не ломать существующие тесты.
2. Постепенно переписывай классы под JUnit 5, начиная с новых модулей.
3. Замени аннотации `@RunWith` → `@ExtendWith`, `@Category` → `@Tag`.
4. Используй `assertAll` и `assertThrows` для более выразительных проверок.
5. Настрой `maven-surefire-plugin` или `gradle test` на `useJUnitPlatform()`.
6. Замени использование `@Rule` и `@ClassRule` на соответствующие расширения.

### Конфигурация для поддержки обеих версий

Если нужно поддерживать и JUnit 4, и JUnit 5 одновременно:
...
JUnit 5 — это не просто обновление, а новая философия тестирования в Java. Он объединяет гибкость, читаемость и расширяемость, сохраняя при этом обратную совместимость. Переход на JUnit 5 — это инвестиция в качество тестов и удобство поддержки проектов.
