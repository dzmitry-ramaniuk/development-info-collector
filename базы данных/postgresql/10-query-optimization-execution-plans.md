...
2. [1. Когда нужна оптимизация](#1-когда-нужна-оптимизация)
3. [2. Главные инструменты PostgreSQL](#2-главные-инструменты-postgresql)
   - [Подключение нужных расширений](#подключение-нужных-расширений)
4. [3. Синтаксис EXPLAIN и EXPLAIN ANALYZE](#3-синтаксис-explain-и-explain-analyze)
   - [Важные опции](#важные-опции)
5. [4. Как читать план выполнения](#4-как-читать-план-выполнения)
6. [5. Типовые проблемы и исправления](#5-типовые-проблемы-и-исправления)
   - [Проблема: Seq Scan на большой таблице](#проблема-seq-scan-на-большой-таблице)
   - [Проблема: плохая оценка кардинальности](#проблема-плохая-оценка-кардинальности)
   - [Проблема: сортировки/хеши уходят на диск](#проблема-сортировкихеши-уходят-на-диск)
7. [6. Полезные SQL-шаблоны для диагностики](#6-полезные-sql-шаблоны-для-диагностики)
8. [7. Практический чек-лист оптимизации](#7-практический-чек-лист-оптимизации)
9. [8. Антипаттерны](#8-антипаттерны)
10. [9. Вопросы для самопроверки](#9-вопросы-для-самопроверки)
...
- `pg_stat_statements` — агрегированная статистика по нормализованным запросам;
  это диагностическое расширение, а не механизм квот, очередей или ограничения
  ресурсов.
...
4. Проверьте операции `Sort` и `Hash`:
   - если есть `Disk:`/`temp read/write`, операция использовала временные файлы;
     это повод проверить оценку строк, алгоритм и локальный `work_mem`, но не
     доказательство, что глобальное увеличение `work_mem` безопасно.
...
> Запрос ниже рассчитан на PostgreSQL 13+: начиная с этой версии прежний столбец
> `total_time` разделён на `total_plan_time` и `total_exec_time`. Сбор времени
> планирования требует `pg_stat_statements.track_planning = on`.
